...
const files = {
  ...
  enHome: readFileSync('src/pages/en/index.astro', 'utf8'),
...
const operatorOnlyPattern = /AdSense review window|애드센스|승인 전 전략|수익화|수익형|검색 신호|운영 초점|밀어야|수익 측정/i;
...
const checks = [
  ...
  ['home does not expose operator-only monetization strategy copy', !operatorOnlyPattern.test(files.home)],
  ['English home does not expose operator-only monetization strategy copy', !operatorOnlyPattern.test(files.enHome)],
